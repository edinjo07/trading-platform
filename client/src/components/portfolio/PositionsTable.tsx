...
export default function PositionsTable() {
  ...
  const handleClose = async (e: React.MouseEvent, id: string, symbol: string) => {
    e.stopPropagation()
    setClosingId(id)
    try {
      await closePosition(id)
      addToast({ title: 'Position Closed', message: `${symbol} position closed at market`, variant: 'info', duration: 3500 })
    } catch (err) {
          ...
          {positions.map((pos) => {
            ...
            return (
              ...
              >
                ...
                <td className="px-4 py-3 text-right text-text-secondary tabular">{formatPrice(pos.avg_price, pos.symbol)}</td>
